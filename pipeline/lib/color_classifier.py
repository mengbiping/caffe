...
def main() :
  ...
  parser.add_argument("--color_ref_dir", default="color_32")
  parser.add_argument("--image_dir", default="../../../crawler")
  parser.add_argument("--result_dir", default="../../result_amazon_190k/")
  ...
  for name in sorted(os.listdir(args.image_dir)) :
    if not name.endswith('.jpg'):# search for all the jpg formate images for processing
      continue
    if name < '137599.jpg':# search for all the jpg formate images for processing
      count += 1
      continue
    ...
    for i in range(len(top_three_color)) :
      ...
      print "Top %d color: %s %f" % (i + 1,
              ...
              color_histogram[current_index] / (totol_foreground_pixels+0.1))
